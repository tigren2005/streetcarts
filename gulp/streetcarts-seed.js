...
function deleteUserAccounts(users) {
...
function createFoodcarts(foodcartsData, usersData) {
    console.log('\n\Creating foodcarts.');
    ...
    if (foodcartsData.length > 0) {
    
        async.forEachOf(foodcartsData, function (foodcarts, key, callback) {
...
function deleteFoodcarts(foodcartsData, usersData) {
    ...
    if (foodcartsData.foodcarts.length > 0) {
        ...
        function (error) {
            if (error) {
                console.log("Could not delete foodcart: " + 
                    error.message);
					defer.reject(error);
            } else {
                console.log("Deleted foodcart.");
				defer.resolve();
            }
        });
    }
...
module.exports = {
    ...
    createFoodcarts: createFoodcarts,
	deleteFoodcarts: deleteFoodcarts
